...
class Disc(Envelope):
    ...
    def __init__(self, params):
        ...
        r_cavity = np.linspace(Rstar, rin - 1e-5, 2)
...
if __name__ == "__main__":
    ...
    wave = np.logspace(-6, -3, 4)
    Rstar = 1.
    
    params = {
        "Rstar": Rstar,        # Example value for Rstar (stellar radius)
        "rin": 100.,          # Example inner radius
        ...
        "tau":  np.logspace(3, 0, 4), # Example optical depth
    ...
    for i in range(4):
        ...
        c = ax[i].pcolormesh(img.xw, img.yw, intensity[i], cmap="afmhot", norm=LogNorm(clip=True))
        cbar = fig.colorbar(c, ax=ax[i], orientation='vertical', fraction=0.046, pad=0.04)
        

    fig.supxlabel(r"$x \, \mathrm{[R_\odot]}$")
    fig.supylabel(r"$y \, \mathrm{[R_\odot]}$")
    ...
    # img2 = PolarImage(10*params["Rstar"], R, nu, nv, incl, PA, d, wave)
    ...
    #     #c = ax[i].pcolormesh(img2.vw, img2.uw, intensity2[i], cmap="afmhot", norm=LogNorm(), edgecolor="tab:green")
    #     c = ax[i].pcolormesh(img2.vw, img2.uw, intensity2[i], cmap="afmhot", norm=LogNorm(clip=True))
    #     cbar = fig.colorbar(c, ax=ax[i], orientation='vertical', fraction=0.046, pad=0.04)

    #     ax[i].set_yscale("symlog")


    # fig.supxlabel(r"$u \, \mathrm{[R_\odot]}$")
    # fig.supylabel(r"$v \, \mathrm{[R_\odot]}$")
